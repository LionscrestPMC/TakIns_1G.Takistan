...
[] execVM "common\client\ATM_airdrop\functions.sqf";
...
lbSetCurSel [2903, 0];
	...
	_target = player;
	RedOn = _target addAction["<t color='#B40404'>Chemlight Red On</t>", "common\client\ATM_airdrop\atm_chem_on.sqf",["Chemlight_red"],6,false,false,"","_target == ( player)"];
    BlueOn = _target addAction["<t color='#68ccf6'>Chemlight Blue On</t>", "common\client\ATM_airdrop\atm_chem_on.sqf",["Chemlight_blue"],6,false,false,"","_target == ( player)"];
    YellowOn = _target addAction["<t color='#fcf018'>Chemlight Yellow On</t>", "common\client\ATM_airdrop\atm_chem_on.sqf",["Chemlight_yellow"],6,false,false,"","_target == ( player)"];
    GreenOn = _target addAction["<t color='#30fd07'>Chemlight Green On</t>", "common\client\ATM_airdrop\atm_chem_on.sqf",["Chemlight_green"],6,false,false,"","_target == ( player)"];
    IrOn = _target addAction["<t color='#FF00CC'>Strobe IR On</t>", "common\client\ATM_airdrop\atm_chem_on.sqf",["NVG_TargetC"],6,false,false,"","_target == ( player)"];
...
while {(getPos _target select 2) > 2} do {
	if(isTouchingGround _target and player == vehicle player) then{sleep 7.0; player setVariable ["tcb_ais_agony", false, true];
	}
	else{
	playSound "Vent";
	playSound "Vent2";
...
sleep 1;
	...
	0=[_target,_loadout] call Setloadout; player setVariable ["tcb_ais_agony", false, true];
