/*
*	The cleanUp configuration file.
...
pvpfw_cleanUp_abandonRadius = 2500; // If no unit is closer than the specified distance, the removal countdown will start
pvpfw_cleanup_abandondTimer = 300; // The vehicle will be removed after the specified time
pvpfw_cleanUp_dontCleanUpAround = ["westBase"]; //do not clean abandoned vehicles around these markers
pvpfw_cleanUp_dontCleanUpAroundDistance = 300; // Dont clean if the vehicle is in this range of one of the above markers.
...
pvpfw_cleanUp_customRemoveScript = {
	...
	diag_log format["#PVPFW cleanup: deleting object with custom condition %1",typeOf _object];
};