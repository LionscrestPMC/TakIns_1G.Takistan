...
sleep 300; //Inicia el script a los 5 min

#include "config.sqf"

if (pvpfw_cleanup_cleanAbandonded) then{
	[] execVM "common\server\cleanup\cleanAbandonded.sqf"; //starts the module to automatically clean up abandonded (empty) vehicles
};
		...
		if (!isNull _x) then{
			switch(true)do{
				...
				case(!alive _x):{
					...
					if (_x isKindOf "CAManBase") then{
						...
						if (diag_tickTime > (_cleanUpInitTime + pvpfw_cleanUp_bodyTimer)) then{
							deleteVehicle _x;
							diag_log format["#PVPFW cleanup: deleting body. init = %1, current time = %2",_cleanUpInitTime,diag_tickTime];
						};
					}else{
						...
						if (count _nearEntities == 0) then{
							...
							if (_cleanupInitTime == 0) then{
								...
								if (diag_tickTime > (_cleanupInitTime + pvpfw_cleanUp_vehicleTimer)) then{
									deleteVehicle _x;
									diag_log format["#PVPFW cleanup: deleting vehicle. init = %1, current time = %2",_cleanUpInitTime,diag_tickTime];
								};
							};
						}else{
					...
					if (count _nearEntities == 0) then{
						...
						if (_cleanupInitTime == 0) then{
							...
							if (diag_tickTime > (_cleanupInitTime + pvpfw_cleanUp_weaponHolderTimer)) then{
								deleteVehicle _x;
								diag_log format["#PVPFW cleanup: deleting GroundWeaponHolder. init = %1, current time = %2",_cleanUpInitTime,diag_tickTime];
							};
						};
					}else{
						_x setVariable ["pvpfw_cleanup_InitTime",0,false];
				...
				case(_x isKindOf "Ruins_F"):{
					...
					if (count _nearEntities == 0) then{
						deleteVehicle _x;
						diag_log format["#PVPFW cleanup: deleting destroyed building. type = %1",_type];
					};
				};
				#endif
				...
				case(_x isKindOf "SmokeShell"):{ //actually for chemlights, but they area child of the smokeShell class
					...
					if (_index == -1) then{
						...
						if (diag_tickTime > (_chemLightSetTime + pvpfw_cleanUp_chemLightTimer)) then{
							deleteVehicle _x;
							diag_log format["#PVPFW cleanup: deleting chemlight"];
						};
					};
				};
				case(_x isKindOf "PipeBombBase" || _x isKindOf "ClaymoreDirectionalMine_Remote_Ammo"):{
					...
					if (_index == -1) then{
						...
						if (diag_tickTime > (_pipeBombSetTime + pvpfw_cleanUp_pipeBombTimer)) then{
							deleteVehicle _x;
							diag_log format["#PVPFW cleanup: deleting remotBomb"];
						};
					};
				};
				#endif
